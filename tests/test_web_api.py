...
code, j = call("api/nodes/rename", {"mac": "02:00:00:00:00:09", "nickname": "the trusty one"})
ok("nodes: renaming a paired node works", code == 200 and j["ok"] and j["paired"][0]["name"] == "the trusty one")
code, j = call("api/nodes/rename", {"mac": "02:00:00:00:00:09", "nickname": ""})
ok("nodes: an empty nickname falls back to the node's own name", code == 200 and j["ok"] and j["paired"][0]["name"] == "node-x")
code, j = call("api/nodes/rename", {"mac": "02:00:00:00:00:99", "nickname": "nobody"})
ok("nodes: renaming an unpaired mac fails cleanly, not a crash", code == 200 and j["ok"] is False)
