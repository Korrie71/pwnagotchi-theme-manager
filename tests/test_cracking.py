...
ok("the summary counts add up", {k: v for k, v in summary.items() if k not in ("junk", "located")} == {"total": 4, "cracked": 1, "uploaded": 0, "queued": 2, "invalid": 1, "unknown": 0})
...
# ---------------------------------------------------------------- location memory (the gps plugin's own sidecar files)
reset()
touch("HasLoc_aabbccddee07.pcapng")
json.dump({"Latitude": 48.85837, "Longitude": 2.29448, "Altitude": 35.0}, open(os.path.join(HS, "HasLoc_aabbccddee07.gps.json"), "w"))
touch("NoLoc_aabbccddee08.pcapng")
touch("ZeroFix_aabbccddee09.pcapng")
json.dump({"Latitude": 0, "Longitude": 0}, open(os.path.join(HS, "ZeroFix_aabbccddee09.gps.json"), "w"))
touch("BrokenLoc_aabbccddee10.pcapng")
open(os.path.join(HS, "BrokenLoc_aabbccddee10.gps.json"), "w").write("{not json")
T._slow.clear()
rows = {r["file"]: r for r in T.crack_rows()}
ok("a handshake with a gps sidecar gets its location", rows["HasLoc_aabbccddee07.pcapng"]["loc"] == {"lat": 48.85837, "lon": 2.29448})
ok("no sidecar file: no location, not an error", rows["NoLoc_aabbccddee08.pcapng"]["loc"] is None)
ok("a 0,0 placeholder (no fix yet) doesn't count as a location", rows["ZeroFix_aabbccddee09.pcapng"]["loc"] is None)
ok("a damaged sidecar file is ignored, not a crash", rows["BrokenLoc_aabbccddee10.pcapng"]["loc"] is None)
summary = T.crack_summary()
ok("the summary counts located handshakes", summary["located"] == 1, summary)
ok("...only those with a real fix", summary["total"] == 4 and summary["located"] == 1)
r = cli("cracking", "list")
ok("CLI: cracking list shows the coordinates", "48.8584,2.2945" in r.stdout, r.stdout)
r = cli("cracking", "list")
ok("...but not for a handshake with no location", "NoLoc" in r.stdout and r.stdout.count("@") == 1)
...
finger5 = Panel(tm5)
finger5.calibrate()
tm5.open_menu("list", "crack")
tm5._toast = None
hit = next(r for r in T.menu_hits(tm5._menu) if r[1] == ("crackrow", "HasLoc_aabbccddee07.pcapng"))
finger5.tap_rect(hit[0])
ok("tapping a located handshake shows where it was", tm5._toast and "48.8584" in tm5._toast[0], tm5._toast)
